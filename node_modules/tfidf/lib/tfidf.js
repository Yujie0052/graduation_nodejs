...
exports.tf = function tf(words, stopWords) {
    return preProcess(words, stopWords)
    // Reduce
        .reduce(exports.reduce, {});
};

function preProcess(words, stopWords) {
    return words
    // Normalize words
...
exports.analyze = function analyze(corpus, _stopWords) {
    ...
    return {
        tfidf: function(doc) {
            ...
            var word_tfidf = [];
            var words = unique(preProcess(exports.tokenize(doc), stopWords));
            words.forEach(function (t) {
                term = exports.normalize(t);
                var tfidf = 0;
                if(term in docs[h])
                    tfidf = docs[h][term] * exports.idf(D, dted[term]);
                word_tfidf.push({'term': term, 'tfidf': tfidf});
            });
            return word_tfidf;
        },
        asJSON: function() {
